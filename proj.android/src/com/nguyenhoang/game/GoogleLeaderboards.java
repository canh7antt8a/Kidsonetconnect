	...
	public void unlock(final int score){
		mActivity.runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if(gameHelper.isSignedIn()){
					if(score == 1){
						Games.Achievements.unlock(gameHelper.getApiClient(), "CgkIirTtiJgaEAIQAA");
					}
					if(score == 4){
						Games.Achievements.unlock(gameHelper.getApiClient(), "CgkIirTtiJgaEAIQAQ");
					}
					if(score == 8){
						Games.Achievements.unlock(gameHelper.getApiClient(), "CgkIirTtiJgaEAIQAg");
					}
					if(score == 12){
						Games.Achievements.unlock(gameHelper.getApiClient(), "CgkIirTtiJgaEAIQAw");
					}
					if(score == 15){
						Games.Achievements.unlock(gameHelper.getApiClient(), "CgkIirTtiJgaEAIQBA");
					}
